...
void gtPushMatrix() {
  ...
  // (TODO)
  Matrix lastMatrix = mat_stack.get(mat_stack.size() - 1);
  Matrix copyMatrix = mat_copy(lastMatrix);
  this.mat_stack.add(copyMatrix);
}
...
void gtPopMatrix() {
  ...
  if (this.mat_stack.size() == 1) {
    ...
    this.mat_stack.remove(this.mat_stack.size() - 1);
  }
}
...
void updateTopMatrix(Matrix m) {
  ...
  // (TODO)
  Matrix ctm = this.mat_stack.get(this.mat_stack.size() - 1);
  Matrix new_ctm = mat_multiply(ctm, m);
  this.mat_stack.set(this.mat_stack.size() - 1, new_ctm);
}
...
void gtTranslate(float x, float y, float z) {
  ...
  translateMatrix.values[0][3] = x;
  translateMatrix.values[1][3] = y;
  translateMatrix.values[2][3] = z;
  
  Matrix result = mat_multiply(ctm, translateMatrix);
  this.mat_stack.set(this.mat_stack.size() - 1, result);
}
...
void gtScale(float x, float y, float z) {
  ...
  Matrix result = mat_multiply(ctm, scaleMatrix);
  this.mat_stack.set(this.mat_stack.size() - 1, result); 
}
...
void gtRotateX(float theta) {
  ...
  rotateX.values[1][1] = (float) Math.cos(theta);
  rotateX.values[2][1] = (float) Math.sin(theta);
  rotateX.values[1][2] = (float) -Math.sin(theta);
  rotateX.values[2][2] = (float) Math.cos(theta);
  updateTopMatrix(rotateX);
}
...
void gtRotateY(float theta) {
  ...
  theta = (float)Math.toRadians(theta);
  rotateY.values[0][0] = (float) Math.cos(theta);
  rotateY.values[0][2] = (float) Math.sin(theta);
  rotateY.values[2][0] = (float) -Math.sin(theta);
  rotateY.values[2][2] = (float) Math.cos(theta);
  updateTopMatrix(rotateY);
}
...
void gtRotateZ(float theta) {
  ...
  theta = (float)Math.toRadians(theta);
  rotateZ.values[0][0] = (float) Math.cos(theta);
  rotateZ.values[1][0] = (float) Math.sin(theta);
  rotateZ.values[0][1] = -(float) Math.sin(theta);
  rotateZ.values[1][1] = (float) Math.cos(theta);
  updateTopMatrix(rotateZ);
}
