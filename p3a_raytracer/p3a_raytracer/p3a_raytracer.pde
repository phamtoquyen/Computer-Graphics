...
void render_scene() {
  for (int y = 0; y < height; y++) {
    for (int x = 0; x < width; x++) {
      ...
      float U = (float) ((float)2 * x / width) - 1;
      float V = -((float) ((float)2*y/ height)) + 1;
      ...
      //PVector dirVec = uvw[2].copy().mult(-d).add(uvw[1].copy().mult(V)).add(uvw[0].copy().mult(U)).normalize();
      PVector dirVec = new PVector(
        -d * uvw[2].x + V * uvw[1].x + U * uvw[0].x,
        -d * uvw[2].y + V * uvw[1].y + U * uvw[0].y,
        -d * uvw[2].z + V * uvw[1].z + U * uvw[0].z);
    dirVec.normalize();
 ...
 */
Hit rayIntersectScene(Ray ray) { // add @shape as param
  ...
  for (Shape shape : shapes) {
    Hit currentHit = shape.intersectRay(ray);
    if (currentHit != null) {
      PVector interPoint = currentHit.interPoint;
      float distFromEye = sqrt(pow((eyeVec.x - interPoint.x), 2) + pow((eyeVec.y - interPoint.y), 2) + pow((eyeVec.z - interPoint.z), 2));
      if (distFromEye < minT) {
        minT = distFromEye;
        minHit = currentHit;
      }
...
PVector shadeHelper(Hit sceneHit) {
  ...
  for (Light currLight: lights) {
    ...
    PVector L = PVector.sub(currLight.pos, sceneHit.interPoint).normalize(); // If you know the collision point, you can compare it against the center of the sphere to compute the norm. Don't forget to normalize this before return.
    ...
    float diffuseCoefficient = max(0, L.dot(N));
    totalCol.x += currShapeDifCol.x * currLight.col.x * diffuseCoefficient;
    totalCol.y += currShapeDifCol.y * currLight.col.y * diffuseCoefficient;
    totalCol.z += currShapeDifCol.z * currLight.col.z * diffuseCoefficient;
    // ===== (TODO) Stage 4 ends =======
  }
