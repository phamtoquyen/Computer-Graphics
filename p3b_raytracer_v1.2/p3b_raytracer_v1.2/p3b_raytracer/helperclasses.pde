...
public class Triangle extends Shape {
  ...
  public Hit intersectRay(Ray ray) 
  {
    ...
    // ===== (TODO) Stage 1 begins =======    
    PVector intersection = null; // (TODO 1-1) Compute the intersection point using the ray class and the computed t value.
    float interX = ray.origin.x + t * ray.direction.x;
    float interY = ray.origin.y + t * ray.direction.y;
    float interZ = ray.origin.z + t * ray.direction.z;
    intersection = new PVector(interX, interY, interZ);
    
    PVector v1_v2 = PVector.sub(v2, v1);
    PVector v2_v3 = PVector.sub(v3, v2);
    PVector v3_v1 = PVector.sub(v1, v3);
    
    PVector p_v1 = PVector.sub(intersection, v1);
    PVector p_v2 = PVector.sub(intersection, v2);
    PVector p_v3 = PVector.sub(intersection, v3);
    
   
    float triple1 = PVector.dot(v1_v2.cross(p_v1), this.normal);
    float triple2 = PVector.dot(v2_v3.cross(p_v2), this.normal);
    float triple3 = PVector.dot(v3_v1.cross(p_v3), this.normal);
    ...
    if ((triple1 > 0) && (triple2 > 0) && (triple3 > 0)){
      hit = true;
    }else if ((triple1 < 0) && (triple2 < 0) && (triple3 < 0)){
      hit = true;
    }else {
      hit = false;
    }
