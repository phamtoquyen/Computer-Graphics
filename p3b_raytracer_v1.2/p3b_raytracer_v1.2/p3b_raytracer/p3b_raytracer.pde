...
PVector shadeHelper(Hit sceneHit, int depth) {
  ...
  if (Debug.DO_AMBIENT)
  {
    ...
    PVector ambientColor_contribution = sceneHit.hitShape.mat.ambColor;
    totalCol = PVector.add(totalCol, ambientColor_contribution);
  ...
  if (Debug.DO_REFLECTION && depth > 0 && sceneHit.hitShape.mat.kRefl > 0)
  {
    ...
    //PVector dir = sceneHit.ray.direction.mult(-1);
    //PVector dir = PVector.mult(sceneHit.ray.direction, -1);
    ////PVector dir = sceneHit.ray.direction.mult(-1);
    //float c1 = sceneHit.norm.dot(dir);
    //float c2 = 2 * c1;
    //PVector c3 = PVector.mult(sceneHit.norm, c2);
    ////PVector c3 = sceneHit.norm.mult(c2);
    //PVector c4 = PVector.add(sceneHit.ray.direction, c3);
    //PVector R = c4.normalize();
     
     
    
    //PVector R = PVector.add(sceneHit.ray.direction, sceneHit.norm.mult(PVector.dot(sceneHit.norm, PVector.mult(sceneHit.ray.direction, -1)) * 2)).normalize(); 
    PVector dir = sceneHit.ray.direction;
    PVector R = PVector.add(dir, PVector.mult(N, 2 * N.dot(PVector.mult(dir, -1)))).normalize();
  ...
  for (Light currLight: lights) {
    ...
    if (Debug.DO_SPECULAR)
    {
      ...
      float specular_coeff = pow(max(0, H.dot(sceneHit.norm)), p); // compute the spectacular coefficient, using H, N, and p
      // ===== (TODO) Stage 2b ends =======
      ...
      //totalCol = PVector.add(totalCol, new PVector(specular_contribution.x * currShapeDifCol.x, specular_contribution.y * currShapeDifCol.y, specular_contribution.z * currShapeDifCol.z));
    }
